...
class HCProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_initial_operations(self):
        """
        Setup the client_obj for interact with remote server
        """
    ...
    def do_GET(self):
        ...
        coap_response = self.client_obj.get(self.coap_uri.path)
    ...
    def do_POST(self):
        ...
        if payload is None:
            ...
            return
        coap_response = self.client_obj.post(self.coap_uri.path, payload)
        self.client_obj.stop()
        logger.info("Server response: %s", coap_response.pretty_print())
        self.set_http_response(coap_response)
    ...
    def do_PUT(self):
        ...
        coap_response = self.client_obj.put(self.coap_uri.path, payload)
        self.client_obj.stop()
    ...
    def do_DELETE(self):
        ...
        """
        self.do_initial_operations()
        coap_response = self.client_obj.delete(self.coap_uri.path)
        self.client_obj.stop()
        logger.debug("Server response: %s", coap_response.pretty_print())
        self.set_http_response(coap_response)
